...
const ExclusionReportTable = (props) => {
  const columns = [{
    ...
    <ReactTable
      ...
      getTrProps={(state, row) => {
        if (row && row.original) {
          return {
            onClick: (e) => {
              const viewUrl = EXCLUSION_REPORTS.EDIT.replace(':id', row.original.id)
                .replace(':tab', 'intro');
...
ExclusionReportTable.defaultProps = {};

ExclusionReportTable.propTypes = {
...
export default ExclusionReportTable;
