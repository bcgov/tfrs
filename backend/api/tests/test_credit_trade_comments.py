...
from api.tests.mixins.credit_trade_relationship \
    import CreditTradeRelationshipMixin, CreditTradeFlowHooksMixin
from .data_creation_utilities import DataCreationUtilities
from .base_test_case import BaseTestCase


class TestAPIComments(BaseTestCase, CreditTradeRelationshipMixin,
    ...
    def test_comment_get_permissions(self):
        """
        Test that Credit Trade Comment GET permissions function correctly
        in all cases
        """
        ...
        expected_results = defaultdict(
            lambda: {'status': status.HTTP_403_FORBIDDEN,
    ...
    def test_comment_filtering(self):
        ...
        expected_results = defaultdict(lambda: {
            ...
            'reason': "Default response is no access (and don't even "
                      "confirm it exists)"
        })
        ...
        for (user, trade) in product(all_users, all_credit_trades):
            ...
            with self.subTest(user=user, trade_id=trade,
                              expected_status=expected_status,
    ...
    def test_comment_post_permissions(self):
        ...
        # Key is (status, rescinded_flag, username,
        # creating_privileged_comment)
        ...
        expected_results[('Not Recommended', True, 'fs_husky', False)] = \
            {
                'status': status.HTTP_201_CREATED,
                'reason': 'Initiator can comment in Not Recommended '
                          'if rescinded'
            }
        ...
        expected_results[('Not Recommended', True, 'fs_shell', False)] = \
            {
                'status': status.HTTP_201_CREATED,
                'reason': 'Respondent can comment in Not Recommended '
                          'if rescinded'
            }
        ...
        expected_results[('Recorded', False, 'gov_analyst', False)] = \
            {
                'status': status.HTTP_201_CREATED,
        ...
        expected_results[('Recorded', False, 'gov_director', True)] = \
            {
                'status': status.HTTP_201_CREATED,
                'reason': 'Director can comment in Not Recommended'
            }

        for (trade, user, privileged) in product(
                created_trades, all_users, [True, False]
        ):
    ...
    def test_put_as_fs_invalid(self):
        """
        Test a Credit Trade Comment PUT by a Fuel supplier for a comment
    ...
    def test_individual_comment_actions(self):
        """
        Test that users can edit comments on a credit trade before the
        status changes but not after.
        """
        ...
        CreatedComment = namedtuple(
            'CreatedComment',
            ['id', 'creating_user_relationship']
        )
        ...
        def check_before(cr: self.PreChangeRecord):
            ...
            if cr.trade_id:
                ...
                for comment_action in to_check:
                    if comment_action.status == cr.current_status and \
                            comment_action.rescinded == cr.rescinded:
                        ...
                        comments_to_check.append(
                            CreatedComment(response_data['id'],
                ...
                for comment in comments_to_check:
                    with self.subTest(
                            "Validating that comment can be edited by the "
                            "creator until the trade status changes",
        ...
        def check_after(cr: self.ChangeRecord):
            ...
            # Assert that for each comment we created, the creator can edit
            # the comment until the status changes
            for comment in comments_to_check:
                with self.subTest(
                        "Validating that comment cannot be edited after the trade status changes",
                        comment_id=comment.id,
                        creating_user_relationship=comment.creating_user_relationship
                ):
