...
class TestComplianceReporting(BaseTestCase):
    ...
    def test_organization_balance_never_below_zero(self):
        ...
        organization_balance = OrganizationBalance.objects.get(organization=organization)
        initial_balance = organization_balance.validated_credits
        ...
        # Try to make a transaction that is greater than the available balance
        transaction_amount = initial_balance + 1  # amount greater than available balance
        
        
        # Here you would call the method or endpoint that performs the credit transaction
        # For example:
        response = self.clients['fs_user_1'].post(
            '/api/your_endpoint',  # Replace with your actual endpoint
            content_type='application/json',
            data=json.dumps({'report_id': report_id, 'transaction_amount': transaction_amount})
        )
        
        # Asserts
        self.assertEqual(response.status_code, status.HTTP_200_OK)  # or whatever status code you expect
        
        updated_organization_balance = OrganizationBalance.objects.get(organization=organization)
        
        # Ensure that the organization balance is zero and not negative
        self.assertEqual(updated_organization_balance.validated_credits, 0)
        ...
        self.assertEqual(response.data['transaction_amount'], initial_balance)

        Organization.objects.get_by_natural_key(
                    "Test Org 1")
        self.clients['fs_user_1']
