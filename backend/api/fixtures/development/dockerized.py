...
class DockerEnvironment(OperationalDataScript):
    ...
    def run(self):
        ...
        User(email='fs3@email.com', authorization_guid=uuid.uuid4(), username='fs3', authorization_id='fs3', first_name='FS3', last_name='Supplier', display_name='Third Fuel Supplier', authorization_directory='BCeID', organization=Organization.objects.get_by_natural_key(self._orgs[2])).save()
        User(email='analyst@email.com', authorization_guid=uuid.uuid4(), username='analyst', authorization_id='analyst', first_name='Analyst', last_name='Government',   display_name='(Analyst)', authorization_directory='IDIR',  organization=Organization.objects.get(id=1)).save()
        User(email='director@email.com', authorization_guid=uuid.uuid4(), username='director', authorization_id='director', first_name='Director', last_name='Government', display_name='(Director)', authorization_directory='IDIR',  organization=Organization.objects.get(id=1)).save()
        User(email='admin@email.com', authorization_guid=uuid.uuid4(), username='admin', authorization_id='admin', first_name='Admin', last_name='Government', display_name='(Admin)', authorization_directory='IDIR', organization=Organization.objects.get(id=1)).save()
        ...
        UserRole(user=User.objects.get(username='admin'), role=Role.objects.get_by_natural_key('Admin')).save()
    ...
    def dump_nginx_config(self):
        ...
        with open('/shared/reverse_proxy.conf', 'w') as f:
            for (i, username) in enumerate(self._usernames):
                conf = 'server {{\n' \
                       ...
                       '\t\tproxy_set_header Sm-UniversalId "{user.authorization_id}";\n' \
                       '\t\tproxy_set_header Smgov-Userguid "{user.authorization_guid}";\n' \
                       '\t\tproxy_set_header Sm-Authdirname "{user.authorization_directory}";\n' \
                       '\t\tproxy_set_header Smgov-Useremail "{user.email}";\n' \
                       '\t\tproxy_set_header Smgov-UserDisplayName "{user.display_name}";\n' \
                       '\t\tproxy_set_header Smgov-USERTYPE "{usertype}";\n' \
                       ...
                       '}}\n\n'.format(
                           ...
                           usertype="Internal" if User.objects.get(username=username).authorization_directory == 'IDIR' else ''
                       )
        ...
        with open('/shared/content/index.html', 'w') as html:
            ...
            html.write('<ul>')
            for (i, username) in enumerate(self._usernames):
                conf = '<li><a target="_blank" href="http://localhost:{port}/">TFRS as {user}</a></li>'.format(port=self._proxied_portbase+i, user=username)
                html.write(conf)
            html.write('</ul>')
