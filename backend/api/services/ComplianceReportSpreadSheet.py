...
class ComplianceReportSpreadsheet(object):
    ...
    def add_exclusion_agreement(self, exclusion_agreement):
        ...
        for record in exclusion_agreement["records"]:
            ...
            worksheet.write(
                row_index, 6, Decimal(record["quantity_not_sold"]), quantity_format
            )
    ...
    def add_schedule_b(self, schedule_b, compliance_period):
        ...
        columns = [
            ...
            "Fuel Class",
            "Provision",
            "Fuel Code or Schedule D Provision",
            "Quantity",
            "Units",
    ...
    def add_schedule_d(self, schedule_d):
        ...
        for sheet in schedule_d["sheets"]:
            ...
            columns = ["Fuel Type", "Feedstock", "Fuel Class"]
            ...
            row_index += 1
            columns = ["Worksheet", "Cell", "Value", "Units", "Description"]
            ...
            # Calculate total intensity for this sheet
            total_intensity = 0
            for output in sheet["outputs"]:
                ...
                worksheet.write(
                    row_index, 1, Decimal(output["intensity"]), value_format
                )
                total_intensity += Decimal(output["intensity"])

            # Add the missing Total (gCO2e/GJ) and Carbon Intensity (gCO2e/MJ) lines
            row_index += 1
            worksheet.write(row_index, 0, "Total (gCO₂e/GJ)", header_style)
            worksheet.write(row_index, 1, total_intensity, value_format)

            row_index += 1
            worksheet.write(row_index, 0, "Carbon Intensity (gCO₂e/MJ)", header_style)
            worksheet.write(row_index, 1, total_intensity / 1000, value_format)
    ...
    def add_schedule_summary(self, summary, compliance_period):
        ...
        if compliance_period >= 2023:
            ...
            line_details["29A"] = (
                "Available compliance unit balance on March 31, "
                + str(int(compliance_period) + 1)
            )
            line_details["29B"] = "Compliance unit balance change from assessment"
            line_details["29C"] = (
                "Available compliance unit balance after assessment on March 31, "
                + str(int(compliance_period) + 1)
            )
            line_details["28"] = (
                "Non-compliance penalty payable ("
                + str(int(Decimal(summary["lines"]["28"]) / 600))
                + " units * $600 CAD per unit)"
            )
        ...
        for line in range(1, 11 + 1):
            ...
            worksheet.write(
                row_index,
                2,
                Decimal(summary["lines"][str(line)]),
        ...
        columns = [
            (
                ...
                if compliance_period < 2023
                else "Low Carbon Fuel Requirement"
            ),
            "Line",
            "Value",
        ]
        ...
        if compliance_period >= 2023:
            ...
            for line in compliance_lines:
                if line != "28" or (line == "28" and summary["lines"][line] > 0):
                    row_index += 1
                    worksheet.write(
                        row_index, 0, line_details[line], description_format
                    )
                    if line.isdigit():
            ...
            for line in range(23, 28 + 1):
                row_index += 1
                worksheet.write(
                    row_index, 0, line_details[str(line)], description_format
                )
                worksheet.write(row_index, 1, "Line {}".format(line))
                worksheet.write(
                    row_index,
        ...
        row_index += 1
        columns = ["Non-compliance Penalty Payable", "Line", "Value"]
        ...
        worksheet.write(
            row_index, 2, Decimal(summary["total_payable"]), currency_format
        )
