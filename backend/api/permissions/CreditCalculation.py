...
class CreditCalculationPermissions(permissions.BasePermission):
    """
    ...
    def has_permission(self, request, view):
        if request.user.has_perm('CREDIT_CALCULATION_MANAGE'):
            return True

        if request.method != 'POST' and \
                request.user.has_perm('CREDIT_CALCULATION_VIEW'):
            return True
