...
docker exec $local_container bash -c 'pg_restore -U tfrs -F t --no-privileges --no-owner -c -d tfrs < /tmp/tfrs.tar' || true
docker exec $local_container bash -c 'pg_restore -U tfrs -F t --no-privileges --no-owner -c -d tfrs < /tmp/tfrs.tar'
echo
